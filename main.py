...
def main():
    ...
    def set_theme(theme_name):
        ...
        theme_manager.load_theme(is_dark)
        main_window.set_theme(is_dark)
        try:
            from PySide6.QtCore import QSettings
            settings = QSettings('PDFTool', 'PDFTool')
            settings.setValue('dark_theme', is_dark)
        except Exception:
            pass
    ...
    light_action = view_menu.addAction('白天 ☀️')
    light_action.setCheckable(True)
    light_action.setChecked(not is_dark)
    light_action.triggered.connect(lambda: set_theme('light'))
    theme_group.addAction(light_action)

    dark_action = view_menu.addAction('晚上 🌙')
    dark_action.setCheckable(True)
    dark_action.setChecked(is_dark)
    dark_action.triggered.connect(lambda: set_theme('dark'))
